...
func leaf_node_cell(node []byte, cell_num int32) []byte {
	return node[LEAF_NODE_HEADER_SIZE+cell_num*LEAF_NODE_CELL_SIZE : LEAF_NODE_HEADER_SIZE+cell_num*LEAF_NODE_CELL_SIZE+LEAF_NODE_CELL_SIZE]
}
...
func is_node_root(node []byte) bool {
	...
	ret, err := utils.BytesToInt(node[IS_ROOT_OFFSET:IS_ROOT_OFFSET+IS_ROOT_SIZE], false)
...
var rowIndex = flag.String("rowIndexB", "1", "the index of row")

// var rowIndexE = flag.Int("rowIndexE", 1, "the end index of row")
